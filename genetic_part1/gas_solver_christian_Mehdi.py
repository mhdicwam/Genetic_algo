...
from mastermind_module import MastermindMatch
from mastermind_module import generateRandomSecret
...
def mutation(this_mutation_rate, this_children):
...
class GASolver:
    ...
    def resetPopulation(self, pop_size=50):
        ...
        for i in range(pop_size):
            chromosome = generateRandomSecret(size=4)
            fitness = self.match.rateGuess(chromosome)
            new_individual = Individual(chromosome, fitness)
            self._population.append(new_individual)

    def test(self):
        print(self.match._secret)
    ...
    def evolveForOneGeneration(self):
        ...
        for i in range(0, len(l_pop), 2):
            ...
            for c in crossover(p1, p2):
                c = mutation(self._mutation_rate, c)
                fitness = self.match.rateGuess(c)
